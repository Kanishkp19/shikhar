...
export function StudyLogModal({ onClose }: { onClose: () => void }) {
  const { getDailyLogEntries, getFormattedDuration, status } =
    useActiveStudyStore();
  ...
  const { resumeTracker, pauseTracker } = useActiveStudyStore();
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div
        ...
        className="relative z-[101] bg-white text-ink rounded-2xl p-6 border border-hairline shadow-2xl space-y-5 max-h-[85vh] flex flex-col box-border"
      >
        {/* Modal Header */}
        <div className="flex items-start justify-between border-b border-hairline pb-4 shrink-0">
          ...
          <button
            onClick={onClose}
            className="text-ink-muted hover:text-ink font-bold text-sm p-1 rounded-lg hover:bg-canvas-soft"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="pt-3 border-t border-hairline shrink-0 flex items-center justify-between text-xs text-ink-muted">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-ink">
              Status: {status === "active" ? "Active Study Tracking" : "Auto-Paused"}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            {status === "active" ? (
              <button
                ...
                className="px-3 py-1.5 bg-canvas-soft border border-hairline text-ink font-bold rounded-lg text-xs hover:bg-hairline transition-colors flex items-center gap-1"
              >
                ⏸ Pause Timer
              </button>
            ) : (
              <button
                ...
                className="px-3 py-1.5 bg-emerald-600 text-white font-bold rounded-lg text-xs hover:bg-emerald-700 transition-colors flex items-center gap-1"
              >
                ▶ Resume Timer
              </button>
            )}
            <button
              onClick={onClose}
              className="px-4 py-1.5 bg-primary text-on-primary font-bold rounded-lg text-xs hover:bg-primary-active"
            >
              Close
            </button>
          </div>
